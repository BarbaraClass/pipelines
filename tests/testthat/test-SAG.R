...
#first step: get format_SAG in the environment (either run full script or import function using dget)

#pipeline_output <- format_SAG(db = paste0(data_path, "/SAG_Barba_Spain"))
pipeline_output <- format_SAG(db = choose_directory())#warnings:  Use of .data in tidyselect expressions was deprecated in tidyselect 1.2.0.

test_that("SAG outputs all files...", {

  expect_true(all("SAG" %in% pipeline_output$Brood_data$PopID))
  expect_true(all("SAG" %in% pipeline_output$Capture_data$CapturePopID))
  expect_true(all("SAG" %in% pipeline_output$Individual_data$PopID))
  expect_true(all("SAG" %in% pipeline_output$Location_data$PopID))
